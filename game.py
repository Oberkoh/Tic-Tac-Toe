...
class TicTacToe():
    ...
    def print_board(self):
        # get the rows and make it a 3x3 box using splicing
        for row in [self.board[i*3 : (i+1)*3] for i in range(3)]:
            print('| ' + ' | '.join(row)+ ' |')

    @staticmethod
    def print_board_nums():
        # 0 | 1 | 2 etc for corresponding boxes
    ...
    def winner(self, square, letter):
        ...
        # check column
        col_ind = square % 3
        column = [self.board[col_ind+i*3] for i in range(3)] # i*3 = row index
